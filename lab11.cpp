...
void render()
{
	...
	case 11:
      ...
      //normalize range to around 1 with out mult
      float fx = (float)x / g.xres * 24.0;
      float fy = (float)y / g.yres * 24.0;
      float v[2] = { fx, fy };
      //pass in x and y get color value
      float ret = noise2(v);
	...
	case 13:
      ...
      float color = (1.0 + sin(fx + (ret/2.0) * 50.0)) / 2.0;
	...
	case 14:
      ...
      //use a for loop to add textures together then average them by loop times
			//Perlin noise
      int c;
      int scale = 1000;
    for (int i = 0; i < 10; i++) {
			  extern float noise2(float vec[2]);
        //normalize range to around 1 with out mult
        float fx = (float)x / g.xres * 3.0 * scale;
        float fy = (float)y / g.yres * 3.0 * scale;
        float v[2] = { fx, fy };
        //pass in x and y get color value
        float ret = noise2(v);
        ...
        //reduce the scale for the next octive
        scale /= 2;

		}
      //divides colors by how many octives used
      c = c /11.0;
      x11.setColor3i(c,c,c);
    }
		break;
    //key 4 is pressed
		//noise with multiple octaves to create marble
 case 15:
    {
      int c;
      int scale = 100;
    for (int i = 0; i < 5; i++) {
        ...
        //normalize range to around 1 with out mult
        float fx = (float)x / g.xres * 3.0 * scale;
        float fy = (float)y / g.yres * 3.0 * scale/10;
        float v[2] = { fx, fy };
        //pass in x and y get color value
        float ret = noise2(v);
    ...
    break;
			//weird wood like texture flickers
      //key 5 is pressed
 case 16:
    {
      int c;
